...
export const createFee = asyncHandler(async (req, res) => {
  ...
  if (issueToClass) {
    const cls = await ClassRoom.findById(issueToClass).populate('students', '_id');
    if (!cls) {
      res.status(404);
  ...
  const fee = await Fee.create({ student, title, amount, dueDate, notes, createdBy: req.user._id });
